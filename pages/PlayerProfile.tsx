...
const Heatmap: React.FC<HeatmapProps> = ({ playerId, position }) => {
  // Define specific heatmaps for our mock players to make it feel premium and realistic
  const getPlayerHeatmap = () => {
    switch (playerId) {
      case '1': // Sanchez (GK)
        return (
          <>
            <div className="absolute bottom-[2%] left-1/2 -translate-x-1/2 w-48 h-24 bg-red-600/50 blur-2xl rounded-full"></div>
            <div className="absolute bottom-[10%] left-1/2 -translate-x-1/2 w-64 h-32 bg-orange-500/30 blur-3xl rounded-full"></div>
          </>
        );
      case '24': // Reece James (RB/RWB)
        return (
          <>
            <div className="absolute top-0 bottom-0 right-[5%] w-24 bg-red-500/30 blur-3xl rounded-full"></div>
            <div className="absolute top-[20%] right-[10%] w-32 h-32 bg-red-600/40 blur-2xl rounded-full"></div>
            <div className="absolute bottom-[20%] right-[10%] w-32 h-32 bg-orange-500/30 blur-2xl rounded-full"></div>
          </>
        );
      case '6': // Thiago Silva (CB)
        return (
          <>
            <div className="absolute bottom-[20%] left-1/2 -translate-x-1/2 w-64 h-40 bg-red-600/40 blur-3xl rounded-full"></div>
            <div className="absolute bottom-[30%] left-1/4 w-32 h-32 bg-orange-500/20 blur-3xl rounded-full"></div>
            <div className="absolute bottom-[30%] right-1/4 w-32 h-32 bg-orange-500/20 blur-3xl rounded-full"></div>
          </>
        );
      case '8': // Enzo (CM)
        return (
          <>
            <div className="absolute top-[30%] bottom-[30%] left-[20%] right-[20%] bg-red-500/30 blur-3xl rounded-full"></div>
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-red-600/40 blur-2xl rounded-full"></div>
            <div className="absolute top-[25%] left-[30%] w-32 h-32 bg-orange-500/30 blur-2xl rounded-full"></div>
          </>
        );
      case '25': // Caicedo (DM)
        return (
          <>
            <div className="absolute bottom-1/3 left-0 right-0 h-40 bg-red-500/20 blur-3xl rounded-full"></div>
            <div className="absolute bottom-[35%] left-1/2 -translate-x-1/2 w-56 h-40 bg-red-600/40 blur-2xl rounded-full"></div>
            <div className="absolute bottom-[40%] right-[20%] w-32 h-32 bg-orange-500/30 blur-2xl rounded-full"></div>
          </>
        );
      case '20': // Cole Palmer (RW/AM)
        return (
          <>
            <div className="absolute top-[10%] right-[10%] w-40 h-40 bg-red-600/50 blur-2xl rounded-full"></div>
            <div className="absolute top-[20%] right-[20%] w-48 h-48 bg-orange-500/30 blur-3xl rounded-full"></div>
            <div className="absolute top-[15%] left-1/2 w-32 h-32 bg-orange-500/20 blur-2xl rounded-full"></div>
          </>
        );
      case '15': // Jackson (ST)
        return (
          <>
            <div className="absolute top-[12%] left-1/2 -translate-x-1/2 w-48 h-32 bg-red-600/50 blur-2xl rounded-full"></div>
            <div className="absolute top-[20%] left-[20%] w-32 h-32 bg-orange-500/30 blur-2xl rounded-full"></div>
            <div className="absolute top-[20%] right-[20%] w-32 h-32 bg-orange-500/30 blur-2xl rounded-full"></div>
          </>
        );
      case '18': // Nkunku (FWD)
        return (
          <>
            <div className="absolute top-[20%] left-[15%] right-[15%] h-40 bg-red-500/30 blur-3xl rounded-full"></div>
            <div className="absolute top-[15%] left-[30%] w-32 h-32 bg-red-600/40 blur-2xl rounded-full"></div>
          </>
        );
      default:
        // Generic Fallbacks based on position
        if (position === 'GK') return <div className="absolute bottom-[5%] left-1/2 -translate-x-1/2 w-40 h-20 bg-red-600/40 blur-2xl rounded-full"></div>;
        if (position === 'DEF') return <div className="absolute bottom-1/3 left-0 right-0 h-32 bg-red-500/30 blur-3xl"></div>;
        if (position === 'MID') return <div className="absolute top-1/3 bottom-1/3 left-[20%] right-[20%] bg-red-500/30 blur-3xl rounded-full"></div>;
        if (position === 'FWD') return <div className="absolute top-[15%] left-1/2 -translate-x-1/2 w-56 h-40 bg-red-600/40 blur-2xl rounded-full"></div>;
        return <div className="absolute inset-0 bg-red-500/10 blur-3xl"></div>;
    }
  };

  return (
    <div className="relative w-full aspect-[2/3] bg-emerald-600 rounded-lg border-2 border-emerald-700 overflow-hidden shadow-inner">
      ...
      <div className="absolute inset-x-4 top-4 bottom-4 border-2 border-white/40"></div>
      
      {/* Halfway Line */}
      <div className="absolute top-1/2 left-4 right-4 h-px bg-white/40"></div>
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-full border-2 border-white/40"></div>
      
      {/* Penalty Areas */}
      {/* Top (Attacking) */}
      <div className="absolute top-4 left-1/2 -translate-x-1/2 w-1/2 h-1/6 border-x-2 border-b-2 border-white/40 bg-white/5"></div>
      <div className="absolute top-4 left-1/2 -translate-x-1/2 w-1/4 h-[8%] border-x-2 border-b-2 border-white/40"></div>
      
      {/* Bottom (Defending) */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-1/2 h-1/6 border-x-2 border-t-2 border-white/40 bg-white/5"></div>
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-1/4 h-[8%] border-x-2 border-t-2 border-white/40"></div>
      ...
      <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col items-center gap-1 opacity-50">
         <div className="w-0 h-0 border-l-[4px] border-l-transparent border-r-[4px] border-r-transparent border-b-[8px] border-b-white"></div>
         <div className="text-[8px] font-bold text-white uppercase vertical-rl tracking-widest writing-mode-vertical">Attack</div>
      </div>
    </div>
  );
};
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
           ...
           <section className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
              ...
              <Heatmap playerId={player.id} position={player.position} />
              <div className="mt-4 flex items-center justify-center gap-4 text-xs text-gray-400">
                 <div className="flex items-center gap-1">
                   <div className="w-2 h-2 rounded-full bg-red-500 opacity-50"></div> High Activity
                 </div>
                 <div className="flex items-center gap-1">
                   <div className="w-2 h-2 rounded-full bg-emerald-200"></div> Pitch
                 </div>
              </div>
           </section>
